...
func (c *Catalog) CreateLibrary(ctx context.Context, lib Library) (*Library, error) {
	if lib.Layout == "" {
		lib.Layout = "books_in_folder"
	}
	if lib.DefaultView == "" {
		lib.DefaultView = ViewHybrid
	}
	res, err := c.db.ExecContext(ctx,
		`INSERT INTO libraries(name, root, layout, default_view, created_at)
		 VALUES(?,?,?,?,?)`, lib.Name, lib.Root, lib.Layout, lib.DefaultView, c.ts())
	if err != nil {
		return nil, err
	}
...
func (c *Catalog) UpsertLibraryByName(ctx context.Context, lib Library) (*Library, error) {
	...
	if lib.Layout == "" {
		lib.Layout = existing.Layout
	}
	if lib.DefaultView == "" {
		lib.DefaultView = existing.DefaultView
	}
	_, err = c.db.ExecContext(ctx,
		`UPDATE libraries SET root = ?, layout = ?, default_view = ? WHERE id = ?`,
		lib.Root, lib.Layout, lib.DefaultView, existing.ID)
	if err != nil {
		return nil, err
	}
	lib.ID = existing.ID
...
// Changing the layout or root makes the index stale, so callers should trigger
// a rescan afterward.
func (c *Catalog) UpdateLibrary(ctx context.Context, id int64, in Library) (*Library, error) {
	...
	if _, err := c.db.ExecContext(ctx,
		`UPDATE libraries SET name = ?, root = ?, layout = ?, default_view = ? WHERE id = ?`,
		existing.Name, existing.Root, existing.Layout, existing.DefaultView, id); err != nil {
		return nil, err
	}
	return existing, nil
}
...
func scanLibrary(row interface{ Scan(...any) error }) (*Library, error) {
	var l Library
	if err := row.Scan(&l.ID, &l.Name, &l.Root, &l.Layout, &l.DefaultView); err != nil {
		return nil, err
	}
	return &l, nil
}
...
func (c *Catalog) GetLibrary(ctx context.Context, id int64) (*Library, error) {
	row := c.db.QueryRowContext(ctx,
		`SELECT id, name, root, layout, default_view FROM libraries WHERE id = ?`, id)
...
func (c *Catalog) GetLibraryByName(ctx context.Context, name string) (*Library, error) {
	row := c.db.QueryRowContext(ctx,
		`SELECT id, name, root, layout, default_view FROM libraries WHERE name = ?`, name)
	l, err := scanLibrary(row)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, ErrNotFound
	}
...
func (c *Catalog) ListLibraries(ctx context.Context) ([]Library, error) {
	rows, err := c.db.QueryContext(ctx,
		`SELECT id, name, root, layout, default_view FROM libraries ORDER BY name`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
